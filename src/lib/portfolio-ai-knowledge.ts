...
export const projectsData = {
  ...
  "Forge": {
    description: "An AI-powered bulk payment and disbursement platform for African businesses",
    tech: ["Python", ".NET", "React", "PostgreSQL"],
    role: "Founder & CEO",
    features: [
      "Python AI engine that validates and auto-corrects bank account details",
      "Fixes wrong account numbers, mismatched names, and duplicates",
      "Bank-name normalisation",
      "Bulk disbursement with a near-zero failure rate"
    ],
    status: "Pre-Launch",
    impact: "Saves businesses weeks of manual data cleaning by disbursing clean, validated payment data reliably"
  },
  "KrediNou": {
    description: "A cross-border fintech super-app for the Haitian diaspora across North America, the Dominican Republic, and Mexico",
    tech: ["Next.js 16", "TypeScript", "Prisma 7", "PostgreSQL", "NextAuth", "Pusher"],
    role: "Lead Developer",
    features: [
      "Multi-currency wallets (USD, HTG, DOP, MXN) with instant P2P transfers",
      "International remittance over live FX corridors and an agent cash-in/cash-out network",
      "Merchant and marketplace suite, ride-hailing, food ordering, virtual cards, and micro-loans",
      "Auditable double-entry ledger with atomic, pessimistically-locked money movement",
      "NextAuth, RBAC, 2FA/OTP, brute-force protection, and KrediSafe escrow"
    ],
    status: "Live",
...
export const responsePatterns: Record<string, string[]> = {
  ...
  feelings: [
    ...
    "Fantastic! Ready to help you explore Muhammad's portfolio. What catches your interest - his AI work, full-stack projects, or maybe his startup Forge?",
  ...
  // Hobbies/Personal
  personal: [
    "Muhammad is passionate about technology and building products that solve real problems. When not coding, he's likely learning about new AI developments or working on his startup Forge. His drive to create impactful software is evident in all his projects.",
    "He's deeply passionate about technology, AI, and entrepreneurship. His personal projects and startup Forge reflect his commitment to building meaningful software solutions."
  ],
  ...
  // Startup/Forge
  startup: [
    `Muhammad founded ${personalInfo.startup} with a mission of ${personalInfo.startupMission}. Forge is an AI-powered bulk payment and disbursement platform for African businesses: a Python AI engine validates and auto-corrects bank account details (wrong numbers, mismatched names, duplicates, bank-name normalisation), then disburses clean data with a near-zero failure rate. It's built using Python (AI layer), .NET, React, and PostgreSQL. He's also building Anvil, a cross-border fintech mobile app for seamless international money transfers, where recipients automatically receive funds in their local currency with no P2P exchange or manual conversion. Anvil is built in Flutter with a .NET backend and PostgreSQL, shipped as a product under Forge for the African remittance corridor.`
  ],
  ...
  unknown: [
    ...
    "That's an interesting question! While I might not have that specific info, I can tell you a lot about Muhammad's technical work, his startup Forge, or help you connect with him. What sounds good?",
  ...
  startups: [
    "Muhammad loves working with startups! As a founder of Forge himself, he understands the startup journey - moving fast, building MVPs, and iterating based on feedback. He's helped startups build:\n\n- MVPs and prototypes\n- Scalable SaaS platforms\n- Multi-tenant systems\n\nUse the **Contact Form** to discuss your startup idea!",
    "Great fit! Muhammad is a startup founder himself (Forge) and understands the unique needs of early-stage companies. He can help you build fast, scale smart, and iterate quickly. Reach out via the **Contact Form**!"
  ],
  ...
  // Industries & domains
  industries: [
    "Muhammad has worked across multiple industries:\n\n- **Healthcare**: Nubenta Care (hospital management)\n- **Education**: SmartEd ERP (school system)\n- **Construction**: BuildTrack Pro (expense tracking)\n- **Logistics**: Bus Tracker (transportation)\n- **Fintech**: Forge (AI-powered bulk payments), BulkPay (payroll system)\n\nHe adapts quickly to new domains!",
    "He's worked in healthcare, education, finance, logistics, and fintech. Muhammad learns domain-specific requirements fast and builds solutions that fit industry needs. What's your industry?"
  ],
  ...
  futurePlans: [
    "Muhammad has ambitious goals:\n\n**Short-term**: Launch Forge and grow its user base among African businesses\n**Medium-term**: Become a recognized name in AI and fintech development\n**Long-term**: Build a globally recognized tech company from Nigeria, proving that world-class innovation can come from anywhere\n\nHe's driven by the mission to show what Nigerian developers can achieve!",
    "His big dream? To build a globally recognized startup from Africa. Muhammad is focused on growing Forge, mastering AI/ML, and creating technology that impacts millions worldwide."
  ],
...
export function detectIntent(message: string): string {
  ...
  if (lowerMessage.length < 10 && lowerMessage.split(' ').length <= 2 &&
      ...
      !/\b(fail|age|old|about|skill|project|experience|forge|anvil|contact|available)\b/.test(lowerMessage)) {
  ...
  // Startup/Forge
  if (/forge|anvil|startup|found|company|business|entrepreneur/.test(lowerMessage)) {
    return 'startup';
  }
  ...
  if (/nubenta|forge|anvil|invotrek|buildtrack|smarted|bulkpay|bus\s*tracker|rewardify|rental|project/.test(lowerMessage) ||
  ...
  // Forge vision - enhanced understanding
  if (/forge|anvil|what.*forge|tell.*forge|about.*forge|forge.*vision|forge.*goal/.test(lowerMessage) ||
      /startup.*vision|what.*building|flagship.*product|main.*project/.test(lowerMessage) ||
      /bulk\s*payment.*app|disbursement.*platform|fintech.*tool/.test(lowerMessage)) {
    return 'forgeVision';
  }
...
function extractKeyConceptsFromMessage(message: string): string[] {
  ...
  const entities = {
    ...
    // Projects/work
    work: /\b(project|build|built|made|created|develop|work|portfolio|product|startup|company|forge|anvil|nubenta)\b/g,
    // Business/hiring
...
export function generateSmartSuggestions(
  ...
  const suggestionFlows: Record<string, SmartSuggestion[]> = {
    ...
    forgeVision: [
      {
        question: "What problem does Forge solve?",
      ...
      {
        question: "Can he build something similar for me?",
        reasoning: "Apply Forge experience to user's project",
        priority: 'high'
      }
    ],
    whyHire: [
...
function updateConversationState(
  ...
  const allTopics = ['skills', 'projects', 'whyHire', 'pricing', 'availability', 'forgeVision'];
...
export function generateContextSwitchBridge(oldIntent: string, newIntent: string): string {
  const bridges: Record<string, Record<string, string>> = {
    ...
    about: {
      ...
      forgeVision: "Speaking of his journey, let me tell you about his startup Forge:"
    }
  };
